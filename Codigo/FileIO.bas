...
116             .Da√±oArmas = val(BalanceIni.GetValue("MODDANOARMAS", SearchVar))
118             .Da√±oProyectiles = val(BalanceIni.GetValue("MODDANOPROYECTILES", SearchVar))
120             .Da√±oWrestling = val(BalanceIni.GetValue("MODDANOWRESTLING", SearchVar))
...
170     ModDaÒoGolpeCritico = val(BalanceIni.GetValue("EXTRA", "ModDaÒoGolpeCritico"))
...
178     AgregarAConsola "Se cargÛ el balance (Balance.dat)"
...
202                     .Apu√±ala = val(Leer.GetValue(ObjKey, "Apu√±ala"))
...
422                 Case e_OBJType.otDa√±oMagico
...
538             .ForoID = Leer.GetValue(ObjKey, "ID")
                ...
                'CHECK: !!! Esto es provisorio hasta que los de Dateo cambien los valores de string a numerico  -  Nunca m·s papu
                Dim n As Integer
                Dim s As String
...
300                         NpcList(npcIndex).pos.y = NPCs(i).y
                            '  guardo siempre la pos original... puede sernos ˙til ;)
...
310                         Call TraceError(404, "NPC no existe en los .DAT's o est· mal dateado. Posicion: " & Map & "-" & NPCs(i).x & "-" & NPCs(i).y, "ES.CargarMapaFormatoCSM")
                        End If
                    End If
312             Next i
            End If
...
100     If frmMain.Visible Then frmMain.txStatus.Caption = "Cargando info de inicio del server."
        ...
        If Not FileExist(IniPath & "Server.ini", vbArchive) Then
            MsgBox "Se requiere de la configuraciÛn en Server.ini", vbCritical + vbOKOnly
            End
        End If
...
112     MaxLvlCania = val(IniFile.GetValue("PECES", "MaxlvlcaÒa"))
        CountEspecial = 1
...
124             Field = Split(str, "-")
                ...
                If val(Field(3)) = 1 Then
                    ...
                    nivel = val(Field(2))               ' Nivel de caÒa
...
130             nivel = val(Field(2))               ' Nivel de caÒa
...
134             Peces(i).amount = nivel
            ...
            ' Los ordeno segun nivel de caÒa (quick sort)
...
114             For i = 1 To MaxRangoFaccion
                    '<N>Rango=<NivelRequerido>-<AsesinatosRequeridos>-<TÌtulo>
...
Public Function GetElapsedTime() As Single
        ...
        '***********************************************************************
        'Author: Wyrox
        'Obenemos el tiempo (en milisegundos) que pasÛ desde la ultima llamada.
        '***********************************************************************
